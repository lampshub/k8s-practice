...
public class JwtAuthFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        String urlPath = exchange.getRequest().getURI().getRawPath();   //localhost:8080/member-create/member/create 중 member/create만 꺼냄
...
//            admin권한 있어야 하는 url 검증
            if(ADMIN_ONLY_PATH.contains(urlPath) && !role.equals("ADMIN")){
                exchange.getResponse().setStatusCode(HttpStatus.FORBIDDEN);  //403에러 (로그인은 했지만 권한이 없음(admin을 요구하는데 user일때)
...
//            추후 서비스모듈에서 RequestHeader어노테이션을 사용하여 아래 헤더를 꺼낼 쓸수 있음
            ...
            e.printStackTrace();
            exchange.getResponse().setStatusCode(HttpStatus.UNAUTHORIZED);  //401에러 (토큰값이 올바르지 않으면(길이가 짧다던지), 토큰이 없거나, 로그인 안했거나)
